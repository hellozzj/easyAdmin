...
$db_cnf = require CONFIG_DIR.'db_'.APP_ENV.'.php';
extract($_REQUEST);
$title = '增删改查模版生成工具';
if(!isset($database)){
	$database = key($db_cnf);
...
$dataname = $db_cnf[$database]['dbname'];
$db = Db::getSlaveInstance($database);
$tables = $db->get_all("SHOW TABLES");

$db_instance = $db_cnf[$database]['alias'] ?? 'CConstant::DB_MAIN';
...
if(isset($table) && !empty($table) && isset($submit)){
	...
	$mod=$mod?$mod:$table;
	$key_field_arr = $db->get_all("SHOW INDEX FROM $table WHERE Key_name = 'PRIMARY'");
	$key_field = $autoid_field = $key_field_arr[0]['Column_name'];

	// 创建模块目录
	$dir = '../../web/admin/app/'.$mod;
	is_dir($dir) OR mkdir($dir, 0777, true);
	// 创建模块静态文件目录
	$dir_tpl = '../../web/admin/app/'.$mod.'/_tpl';
	is_dir($dir_tpl) OR mkdir($dir_tpl, 0777, true);
    ...
    if(isset($is_ajax)){
        ...
        file_put_contents($dir_tpl.'/'.$file_name.".tpl.htm",$contents);
        ...
        $contents=str_replace('%%?php','<?php',$contents);
        $contents=str_replace('?%%','?>',$contents);
        file_put_contents($dir_tpl.'/sub_'.$file_name.".tpl.htm",$contents);
        ...
        $contents=str_replace('%%?php','<?php',$contents);
        $contents=str_replace('?%%','?>',$contents);
        file_put_contents($dir.'/'.$file_name.".act.php",$contents);
    }else{
        ob_start();
    ...
    if(empty($is_readonly)){
		...
		$contents=str_replace('%%?php','<?php',$contents);
		$contents=str_replace('?%%','?>',$contents);
		file_put_contents($dir_tpl.'/'.$file_name."_edit.tpl.htm",$contents);
		...
		$contents=str_replace('%%?php','<?php',$contents);
		$contents=str_replace('?%%','?>',$contents);
		file_put_contents($dir.'/'.$file_name."_edit.act.php",$contents);
		...
		$contents=str_replace('%%?php','<?php',$contents);
		$contents=str_replace('?%%','?>',$contents);
		file_put_contents($dir.'/'.$file_name."_delete.act.php",$contents);
    }
	?>
	<!-- // 更新到menu中 -->
...
<div id="main" class="wrapper">
	...
	<form method="post">
		...
		<ul>
			<li> 标题</li>
			<li class="floatright">
                <?php $table_status = $db->get_row("SHOW TABLE STATUS LIKE '{$table}'");?>
...
<?php } ?>
		...
		<?php if(isset($table) && !empty($table)){
			$table_struct = $db->get_all("SHOW FULL FIELDS FROM $table");

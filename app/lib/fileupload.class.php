...
namespace app\lib;
  ...
  class FileUpload { 
    ...
    private $allowtype = array('jpg','jpeg','gif','png','txt','apk','ipa'); //设置限制图片文件的类型
        ...
        if($return){
          ...
          foreach($name as $key=>$val){ 
            if($this->setFiles($name[$key], $tmp_name[$key], $size[$key], $error[$key] )) {
              ...
              $fileNames[$key] = $this->path . "/" .$this->newfilename;  
            }          
          }
    ...
    private function setNewFileName() {
      if ($this->israndname) {
        ...
        $this->setOption('newfilename', $this->newfilename.'.'.$this->fileType);
      }
    }

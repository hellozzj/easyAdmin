...
class Db extends PDO{
    ...
    public static function getMasterInstance($db_type, $options = array()){
        ...
        return self::getInstance($db_type, $dsn, CConstant::DB_POS_MASTER, $options);
    }
     ...
     * @param string $db_type 数据库名
     */
    public static function getSlaveInstance($db_type, $options = array()){
        ...
        return self::getInstance($db_type, $dsn, CConstant::DB_POS_SLAVE, $options);
    }
     ...
     * 获取单例
     */
    private static function getInstance($db_type, $dsn, $position, $options){
        try{
            if(!$dsn){
                throw new PDOException("数据库配置错误");
            }
            self::$instance[$db_type][$position] = new self($dsn, $position, $options);
     ...
     */
    public function __construct($dsn, $position, $options){
        $user = $dsn['user'];
        $pass = $dsn['pwd'];
        $charset = $dsn['charset'];
    ...
    private function is_sql_valid($sql){
        ...
        if(preg_match($pattern_slave, $sql) && $this-> _position == CConstant::DB_POS_SLAVE){
            return false;
        }
        $pattern_master = "/SELECT/iA";  
        if(preg_match($pattern_master, $sql) && $this-> _position == CConstant::DB_POS_MASTER){
            return false;
        }
        return true;
    }
